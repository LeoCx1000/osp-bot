...
class banembed(menus.ListPageSource):
    ...
    async def format_page(self, menu, entries):
        embed = discord.Embed(
            title=f"Server bans ({len(entries)})", description="\n".join(entries)
        )
        embed.set_footer(
            text=f"To unban do db.unban [entry]\nMore user info do db.baninfo [entry]"
        )
        return embed
...
class moderation(commands.Cog):
    def __init__(self, bot):
        ...
        with open(r"files/config.yaml") as file:
            ...
            for roleid in full_yaml["StaffRoles"]:
                staff_roles.append(
                    self.bot.get_guild(full_yaml["guildID"]).get_role(roleid)
                )
    ...
    async def pfp(self, ctx, user: typing.Optional[discord.Member]):
        ...
        embed = discord.Embed(color=ctx.me.color)
        embed.description = f"[PNG]({user.display_avatar.replace(format='png', size=2048).url}) **|** [JPG]({user.display_avatar.replace(format='jpg', size=2048).url}) **|** [WEBP]({user.avatar.replace(format='webp', size=2048).url})"
    ...
    async def userinfo(
        ...
        # BIRTHDAY
        current_birthday = await self.bot.db.fetchval(
            "SELECT birthdate FROM userinfo WHERE user_id = $1", member.id
        )
        if current_birthday:
        ...
        embed = discord.Embed(
            color=(member.color if member.color != discord.Colour.default() else None)
        )
        ...
        embed.add_field(
            ...
            value=(self.get_user_badges(member) or "No Badges") + "\u200b",
            inline=True,
        )
        ...
        if perms:
            embed.add_field(
                name=f"{constants.STORE_TAG} Staff Perms:",
                value=f"`{'` `'.join(perms)}`",
                inline=False,
            )
    ...
    async def kick(
        self, ctx, member: typing.Optional[discord.Member] = None, *, reason=None
    ):
        ...
        if member == None:
            ...
            await self.error_message(
                ctx, "I'm not high enough in role hierarchy to kick that member!"
            )
            return
        if member.top_role <= ctx.author.top_role:
            if (
                member.guild_permissions.ban_members == False
                or member.guild_permissions.kick_members == False
            ):
                try:
...
```reason: {reason}```""",
    ...
    async def ban(
        ...
        if member == None:
            ...
            await self.error_message(
                ctx, "I'm not high enough in role hierarchy to ban that member!"
            )
            return
        if member.top_role <= ctx.author.top_role:
            if (
                ...
                try:
                    ...
                    if reason:
                        embed = discord.Embed(
                            description=f"""{ctx.author.mention} banned {member.mention}
```reason: {reason}```""",
                            color=ctx.me.color,
                        )
                    else:
        ...
        *,
        ...
        if member == None:
            if ctx.channel.permissions_for(ctx.author).manage_nicknames:
                await ctx.send(
                    "`.nick [member] (newNick)` - You must specify a member",
                    delete_after=10,
                )
        ...
        if (
            member == ctx.author
            and ctx.channel.permissions_for(ctx.author).change_nickname
        ):
            try:
                await member.edit(nick=new)
                await ctx.send(
                    f"""✏ {ctx.author.mention} nick for {member}
**`{old}`** -> **`{new}`**"""
                )
                try:
                    await ctx.message.delete()
            ...
            if member.top_role >= ctx.author.top_role:
                await self.error_message(
                    ctx, "⚠ Cannot edit nick for members equal or above yourself!"
                )
                return
            try:
                await member.edit(nick=new)
                await ctx.send(
                    f"""✏ {ctx.author.mention} edited nick for **{member}**
**`{old}`** -> **`{new}`**"""
                )
                try:
                    await ctx.message.delete()
            ...
            await self.error_message(
                ctx,
                f"""You can only change your own nick!
> !nick {ctx.author.mention} `<new nick>`""",
    ...
    @commands.group(
        aliases=["purge", "clear", "delete", "clean"],
        description="""```yaml
    ...
    async def cleanup(self, ctx, amount: int = 25):
        ...
        if amount > 25:
            if not ctx.channel.permissions_for(ctx.author).manage_messages:
                await ctx.send(
                    "You must have `manage_messages` permission to perform a search greater than 25"
                )
                return
            if not ctx.channel.permissions_for(ctx.me).manage_messages:
                await ctx.send(
                    "I need the `manage_messages` permission to perform a search greater than 25"
                )
                return
    ...
    async def mute(
        ...
        if member == None:
            ...
            return
        muterole = self.bot.get_guild(self.yaml_data["guildID"]).get_role(
            self.yaml_data["MuteRole"]
        )
        ...
        try:
            ...
            mem_embed.set_author(
                name=f"You've been muted by a moderator",
            ...
            if reason:
                embed = discord.Embed(
                    description=f"""{ctx.author.mention} muted {member.mention} indefinitely...
```reason: {reason}```""",
                    color=ctx.me.color,
                )
            else:
    ...
    async def unmute(
        ...
        try:
            ...
            mem_embed = discord.Embed(color=ctx.me.color)
            mem_embed.set_author(
                name=f"You've been unmuted by a moderator",
...
```reason: {reason}```""",
    ...
    async def denymedia(
        ...
        try:
            ...
            mem_embed = discord.Embed(color=ctx.me.color)
            mem_embed.set_author(
                name=f"You've been denied permissions to send media by a moderator",
                icon_url="https://i.imgur.com/hKNGsMb.png",
            )
            if reason:
                mem_embed.set_footer(text=f"reason: {reason}")
            ...
            if reason:
                embed = discord.Embed(
                    description=f"""{ctx.author.mention} denied media pemrs to {member.mention}
```reason: {reason}```""",
                    color=ctx.me.color,
                )
            else:
    ...
    async def allowmedia(
        ...
        try:
            ...
            mem_embed = discord.Embed(color=ctx.me.color)
            mem_embed.set_author(
                name=f"You've been allowed permissions to send media by a moderator",
                icon_url="https://i.imgur.com/m1MtOVS.png",
            )
            if reason:
                mem_embed.set_footer(text=f"reason: {reason}")
            ...
            if reason:
                embed = discord.Embed(
                    description=f"""{ctx.author.mention} returned media pemrs to {member.mention}
```reason: {reason}```""",
                    color=ctx.me.color,
                )
            else:
    ...
    @commands.has_permissions(manage_messages=True)
    async def lockdown(
        self, ctx, textchannel: typing.Optional[discord.TextChannel], *, reason=None
    ):
    ...
    async def move(
        ...
        if channel == None:
            await self.error_message(
                ctx, "you must specify a channel: .move <amount> <#channel>"
            )
            ctx.command.reset_cooldown(ctx)
            return
        elif channel == ctx.channel:
            await self.error_message(
                ctx, "channel can't be this channel: .move <amount> <#channel>"
            )
            ctx.command.reset_cooldown(ctx)
            return
        if (
            not channel.permissions_for(ctx.guild.me).manage_webhooks
            and not ctx.channel.permissions_for(ctx.me).manage_messages
        ):
        ...
        if amount == None:
            await self.error_message(
                ctx, "you must specify an amount: .move <amount> <#channel>"
            )
            ctx.command.reset_cooldown(ctx)
            return
        elif amount > 20:
        ...
        try:
            webhook = await channel.create_webhook(
                name="DB-Move", reason="created webhook for move command"
            )
        except:
            await ctx.send(
                f"i'm missing manage_webhooks permission in {channel.mention}",
                delete_after=5,
            )
            ctx.command.reset_cooldown(ctx)
            return
    ...
    async def embed(self, ctx: commands.Context, *, flags: TestFlags):
        ...
        **For example:** `--field --name hi hello --value more text --inline no`
        ...
        """
        embed = discord.Embed(
            title=flags.title, description=flags.description, colour=flags.color
        )
        ...
        if any(
            [flags.title, flags.image, flags.description, flags.field, flags.footer]
        ):
            if len(embed) > 6000:
            ...
            raise commands.BadArgument(
                "You must pass at least one of the necessary (`*`) flags!"
            )
