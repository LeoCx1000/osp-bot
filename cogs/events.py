...
class events(commands.Cog):
    def __init__(self, bot):
        ...
        # ------------- YAML STUFF -------------#
        with open(r"files/config.yaml", encoding="utf-8") as file:
        ...
        with open(r"files/triggers.yaml") as triggers:
            self.trigger_words = yaml.full_load(triggers)
    ...
    async def on_guild_channel_create(self, channel):
        if channel.guild.id != self.mguild.id:
            return
        await channel.set_permissions(
            self.blackout, view_channel=False, reason=f"automatic Blackout mode"
        )
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if str(payload.emoji) == "🚪":
            ...
            if payload.channel_id in chids:
                ...
                if not message.author.bot or "opened a ticket" not in message.content:
                    return
                try:
...
You have 5 minutes to do so.""",
                ...
                embed.set_author(
                    name=f"{payload.member} is leaving the ticket",
                ...
                try:
                    ...
                    msg = await self.bot.wait_for(
                        event="message", check=check, timeout=300.0
                    )
                    # msg = discord.Message
...
{msg.content}""",
                ...
                logemb = discord.Embed(
                    color=0xD7342A,
                    title=f"Left ticket #{message.channel.name}",
                    description=f"""
{payload.member.mention} left ticket: {message.channel.mention}""",
                ...
                logemb.set_author(
                    name=str(payload.member), icon_url=payload.member.display_avatar.url
                )
            ...
            if payload.channel_id in chids:
                ...
                embed.clear_fields()
                embed.add_field(
                    name="Actions:", value="Archived! | 🗑 Delete (staff-only)"
                )
                ...
                logemb = discord.Embed(
                    color=0x4286F4,
                    title=f"Ticket #{message.channel.name} archived",
                    description=f"""
                ...
                overwrites = {
                    ...
                    self.ticket_staff: discord.PermissionOverwrite(
                        read_messages=True, manage_messages=False, send_messages=False
                    ),
                }
            ...
            if payload.channel_id in chids:
                ...
                embed.add_field(
                    name="Actions:",
                    value="📁 Archive (staff-only) | 🗑 Delete (staff-only)",
                )
                await message.edit(content=message.content, embed=embed)
                ...
                logemb = discord.Embed(
                    color=0x4286F4,
                    title=f"Ticket #{message.channel.name} closed",
                    description=f"""
            ...
            if payload.channel_id in chids:
                ...
                logemb = discord.Embed(
                    color=0xD7342A,
                    title=f"Ticket #{message.channel.name} deleted",
                    description=f"""
    ...
    async def on_member_join(self, member):
        if member.guild.id != self.yaml_data["guildID"]:
            return
        xxth = make_ordinal(
            sorted(member.guild.members, key=lambda user: member.joined_at).index(
                member
            )
            + 1
        )
        ...
        await self.bot.get_channel(self.yaml_data["WelcomeChannel"]).send(
        ...
        await member.add_roles(
            self.mguild.get_role(860612731102822400),
            self.mguild.get_role(860609206453665792),
            self.mguild.get_role(860613002906697800),
            self.mguild.get_role(860612542619189249),
            self.mguild.get_role(867094081241612308),
            self.mguild.get_role(867096893863624734),
    ...
    async def on_member_update(self, before, after):
        if before.roles != after.roles:
            ...
            if (
                underaged in before.roles
                and overaged not in before.roles
                and overaged in after.roles
                and underaged not in after.roles
            ):
                await channel.send(
                    before.guild.owner.mention,
                    embed=discord.Embed(
                        color=0xFF0000,
                        description=f"""
...
""",
                ...
                overaged in before.roles
                and underaged not in before.roles
                and underaged in after.roles
                and overaged not in after.roles
            ):
                await channel.send(
                    before.guild.owner.mention,
                    embed=discord.Embed(
...
""",
            ...
            if (
                underaged in self.STLbefore.roles
                and overaged not in self.STLbefore.roles
                and overaged in after.roles
                and underaged not in after.roles
            ):
                await channel.send(
...
""",
            ...
            if (
                overaged in self.STLbefore.roles
                and underaged not in self.STLbefore.roles
                and underaged in after.roles
                and overaged not in after.roles
            ):
                await channel.send(
...
""",
    ...
    async def on_trigger_word(self, message):
        ...
        suicide_embed = discord.Embed(
            title="DEPRESSION AND SUICIDAL INTENTIONS", description="", color=0x0066FF
        )
        suicide_embed.add_field(
        ...
        self_injury_embed = discord.Embed(
            title="NONSUICIDAL SELF-INJURY", description="", color=0x0066FF
        )
        self_injury_embed.add_field(
            name="Focus Adolescent Services",
        ...
        self_injury_embed.add_field(
            name="S.A.F.E. Alternatives (Self-Abuse Finally Ends)",
        ...
        # Triggers if any of the suicide_triggers in the triggers.yaml is said
            ...
            for word in self.trigger_words["suicide_triggers"]
        ):
        ...
        # Triggers if any of the non_sudoku_triggers in the triggers.yaml is said
            ...
            word in message.content.lower()
            for word in self.trigger_words["non_sudoku_triggers"]
        ):
    ...
    async def on_d_bump(self, message):
        ...
        if (
            message.guild
            and message.content.startswith("!d bump")
                ...
                if msg.embeds:
                    ...
                    if "wait another" in desc:
                        ...
                        mess = await message.channel.send(
                            f"Would you like to start a timer for {time} minutes?"
                        )
                        ...
                        try:
                            ...
                            if str(reaction.emoji) == "✅":
                                await mess.edit(
                                    content=f"timer for {time} minutes started!"
                                )
                                await mess.clear_reactions()
                                await asyncio.sleep(time * 60)
                                await message.channel.send(
                                    f"{message.author.mention} `!d bump` reminder!"
                                )
